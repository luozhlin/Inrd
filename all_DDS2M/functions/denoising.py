...
def efficient_generalized_steps(pinv_y_0, x, seq, model, b, H_funcs, y_0, sigma_0, etaB, etaA, etaC, mask=None, img_clean = None, logger=None, args=None, config=None, image_folder=None):
    ...
    for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
        ...
        ## The untrained network parameters are not updated until iii>args.start_point, roughly equivalent to starting the backdiffusion process from the args.start_point step args.start_point
        update = True if iii < args.start_point  else True
        if update: 
        ...
        with torch.no_grad():
            ...
            psnr,ssim = quality(x0_t.squeeze().cpu().permute(1,2,0).numpy(), img_clean.squeeze().permute(1,2,0).numpy())

            if psnr_best < psnr:
                psnr_best = psnr
                best = x0_t.squeeze().cpu().permute(1,2,0).numpy()
   
            logger.info('iteration: {}, psnr: {}, ssim: {}, psnr_best: {}'.format(iii, psnr, ssim, psnr_best))
    ...
    psnr, ssim = quality(x_save, img_clean[0,:,:,:].permute(1,2,0).detach().cpu().numpy())

    scio.savemat(
        os.path.join(image_folder, f"x_{iii}.mat"), {'y_0': pinv_y_0, 'x_recon':x_save, 'img_clean':img_clean_save, 'psnr':psnr, 'ssim': ssim, 'x_best':np.clip(x_best,0,1), 'psnr_best':psnr_best}
    )
